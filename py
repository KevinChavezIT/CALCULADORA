...
Public OutputType As String                'CHART/PICTURES/WEB/PRINT
...
Public SyncAxis As Boolean              ' Option that indicates if graphs that show Avgs on Y1 and Min/Max on Y2 will have the same max scale value
Public ShowMacProgress As Boolean       ' Indicates if user wants progress shown when analyzing on a Mac (signficant perf degradation)
Public ShowLinuxCPUUtil As Boolean      ' When analyzing Linux data a value of YES indicates that all of the CPUUTILnnn sheets will be included.  CPUUTIL_ALL will always be included.
...
Public cTop As Double, cLeft As Double, cWidth As Double, cHeight As Double, cTemp As Double
Public csTop1 As Double, csTop2 As Double, csTop3 As Double, csTop4 As Double
...
#If Mac = False And VBA7 Then      ' Excel 2010 or higher on Windows
...
Sub MinMaxStyle(Chart1 As ChartObject, mySheet As Worksheet, numrows As Long, bSync As Boolean)
 
    With Chart1.Chart
        ' @RM - 02/17/2016 - add the maximum to the 2nd Y-axis
         .SeriesCollection.NewSeries
         With .SeriesCollection(3)
            .AxisGroup = 2
            .ChartType = xlLine
            .Border.Weight = xlMedium
            .Values = mySheet.Range("B" & CStr(numrows + 4) & ":" & LastColumn & CStr(numrows + 4))
            .Interior.Color = RGB(0, 128, 0)     ' green
            .Border.Color = RGB(0, 128, 0)       ' green
            .Name = "Max"
         End With
         ' @RM - 02/17/2016 - add the Min to the 2nd Y-axis
         .SeriesCollection.NewSeries
         With .SeriesCollection(4)
            .AxisGroup = 2
            .ChartType = xlLine
            .Border.Weight = xlMedium
            .Values = mySheet.Range("B" & CStr(numrows + 5) & ":" & LastColumn & CStr(numrows + 5))
            .Name = "Min"
         End With
         .Axes(xlValue, xlPrimary).HasTitle = True
         .Axes(xlValue, xlPrimary).AxisTitle.Characters.Text = "Avg"
         .Axes(xlValue, xlSecondary).HasTitle = True
         .Axes(xlValue, xlSecondary).AxisTitle.Characters.Text = "Min/Max"
         
         Dim maximum As Double:
            
         If bSync Then
            maximum = CDbl(Application.WorksheetFunction.Max(mySheet.Range("B" & CStr(numrows + 4) & ":" & LastColumn & CStr(numrows + 4)))) + 1
            .Axes(xlValue, xlPrimary).MaximumScale = maximum
            .Axes(xlValue, xlPrimary).MinimumScale = 0
            .Axes(xlValue, xlSecondary).MaximumScale = maximum
            .Axes(xlValue, xlSecondary).MinimumScale = 0
         End If
                                                             'scale the y-axis
        maximum = .Axes(xlValue, xlPrimary).MaximumScale
        If .Axes(xlValue, xlPrimary).DisplayUnit = xlThousands Then maximum = maximum / 1000
        If maximum > 5181 Then
           .Axes(xlValue, xlPrimary).HasDisplayUnitLabel = True
           If maximum > 3333431 Then
              .Axes(xlValue, xlPrimary).DisplayUnit = xlMillions
           Else
        ...
        Dim Pct As String
        Pct = ""
        If Right(.Axes(xlValue, xlPrimary).TickLabels.NumberFormat, 1) = "%" Then
           Pct = "%"
           maximum = maximum * 10
        End If
        If maximum >= 10 Then
           .Axes(xlValue, xlPrimary).TickLabels.NumberFormat = "0" & Pct
        Else
           .Axes(xlValue, xlPrimary).TickLabels.NumberFormat = "0.0" & Pct
        End If
        
         maximum = .Axes(xlValue, xlSecondary).MaximumScale
        If .Axes(xlValue, xlSecondary).DisplayUnit = xlThousands Then maximum = maximum / 1000
        If maximum > 5181 Then
           .Axes(xlValue, xlSecondary).HasDisplayUnitLabel = True
           If maximum > 3333431 Then
        ...
        End If
        Pct = ""
        If Right(.Axes(xlValue, xlSecondary).TickLabels.NumberFormat, 1) = "%" Then
           Pct = "%"
           maximum = maximum * 10
        End If
        If maximum >= 10 Then
           .Axes(xlValue, xlSecondary).TickLabels.NumberFormat = "0" & Pct
        Else
           .Axes(xlValue, xlSecondary).TickLabels.NumberFormat = "0.0" & Pct
        End If
    End With
End Sub
...
End With
                                        'scale the y-axis
With Chart1.Chart
    If Chart1.Chart.ChartType <> xlAreaStacked100 Then
...
End Sub
                                       'last mod v3.0
Function avgmax(numrows As Long, Sheet1 As Worksheet, DoSort As Boolean) As Range
...
Dim n As Integer
Dim avgmax_rows As Integer
avgmax_rows = 6
If DoSort Then
    avgmax_rows = 7
End If
                                        ' Put in the formulas for avg/max
Set avgmax = Sheet1.Range("A" & CStr(numrows + 2) & ":" & LastColumn & CStr(numrows + avgmax_rows))
...
' @RM 02/17/2016 - no longer tweaking the max value so it looks better on the stacked column graph, instead the max will be on the Y2.
avgmax.Item(3, 2) = "=MAX(B2:B" & CStr(numrows) & ")"
' @RM 02/17/2016 - add minimum value
avgmax.Item(4, 1) = "Min."
avgmax.Item(4, 2) = "=MIN(B2:B" & CStr(numrows) & ")"

If DoSort = True Then
   avgmax.Item(5, 1) = "SortKey"
   avgmax.Item(5, 2) = "=B" & CStr(numrows + 2) & "+ B" & CStr(numrows + 3)
End If


If LastColumn <> "B" Then
   Set MyCells = Sheet1.Range("B" & CStr(numrows + 2) & ":" & LastColumn & CStr(numrows + avgmax_rows))
   MyCells.FillRight
...
' @RM - 2/19/2016 - pivot charts don't work on Excel for Mac
#If Mac Then
    Exit Sub
#End If
...
Call SetStatus("Creating Pivot Chart for " & pSheet)
...
If cWidth = 0 Then
   ...
   Else
        #If Mac Then
        ' @RM 2/12/2019 - By default graphs are too wide on Mac OS
              cWidth = Application.UsableWidth * 0.65
        #Else
...
Sub DiskGraphs(numrows As Long, SectionName As String, DoSort As Boolean)
...
' @RM - 11/12/2018 - per AIX documentation DISKBSIZE is not in KBs, it's the total disk blocks
' DISKBSIZE, Disk Block Size
'    Total number of disk blocks that are read and written over the interval.
If Left(SectionName, 4) = "DISK" And InStr(1, SectionName, "SIZE") > 0 And SectionName <> "DISKBSIZE" Then
   ChartTitle = ChartTitle + " (Kbytes)"
End If
' @RM - 11/12/2018 - the description give to the NMON fields DISKAVGRIO is wrong
' per AIX documentation
...
If SectionName = "DISKAVGRIO" Then
   ChartTitle = "Disk IO Average Reads KBs/xfer"
End If
If SectionName = "DISKAVGWIO" Then
   ChartTitle = "Disk IO Average Writes KBs/xfer"
End If

                                  'Put in the formulas for avg/max
Set DiskData = avgmax(numrows, Sheet1, DoSort)
If DoSort Then
   Sheet1.Range("B1:" & LastColumn & CStr(numrows + 6)).Sort _
    Key1:=Sheet1.Range("B" & CStr(numrows + 6)), Order1:=xlDescending, _
...
' @RM - 02/17/2016 - stacked bar portion (Y1) now only applies to Avg and WAvg (not the Max)
Set DiskData = Sheet1.Range("B" & CStr(numrows + 2) & ":" & Column & CStr(numrows + 3))

ChartTitle = ChartTitle & "  " & RunDate

'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Error GoTo ForceTop1
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop1 + rH * numrows, MyWidth, csHeight)
GoTo ResumeChart1

ForceTop1:
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop1, MyWidth, csHeight)
ResumeChart1:
...
With Chart1.Chart
   ...
   Call MinMaxStyle(Chart1, Sheet1, numrows, SyncAxis)
End With
                                        'produce line graph
Set DiskData = Sheet1.Range("A1:" & Column & CStr(numrows))

'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Error GoTo ForceTop2
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop2 + rH * numrows, MyWidth, csHeight)
GoTo ResumeChart2

ForceTop2:
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop2, MyWidth, csHeight)
ResumeChart2:
...
OutputType = Sheet1.Range("Output").Value
...
SyncAxis = Sheet1.Range("SyncAxis").Value = "YES"                   ' @RM - 02/17/2016
ShowMacProgress = Sheet1.Range("ShowMacProgress").Value = "YES"     ' @RM - 02/19/2016
ShowLinuxCPUUtil = Sheet1.Range("ShowLinuxCPUUtil").Value = "YES"   ' @RM - 02/22/2016
...
If FileName = "" Then
   ' get the list of files to process by prompting the user appropriately on Mac or Windows
   #If Mac Then
       Dim myFiles As String
       myFiles = SelectNMONFile
       FileList = Split(myFiles, ",")
       Numfiles = 1
   
       If VarType(FileList) <> vbBoolean Then
            Numfiles = UBound(FileList) + 1
       End If
   
       'write them to the sheet for sorting
       If Numfiles = 0 Then Exit Sub
       For I = 0 To Numfiles - 1  ' @RM Mac FileList variant is 0-based
            ' @TW - 09/13/2017 - Fixing an issue with Excel 2016 & OSX 10.12.X, where the file path contains the disk name as colons; The Excel and OSX combination only works with / instead of :
            If CInt(Split(Application.Version, ".")(0)) >= 15 Then
                FileList(I) = Replace(FileList(I), ":", "/")
                FileList(I) = Right(FileList(I), Len(FileList(I)) - InStr(1, FileList(I), "/") + 1) '@TW - 09/13/2017 - Remove the volume name at the beginning of the file path
            End If
            ' @TW END
            MyCells.Offset(I + 1, 0) = FileList(I)
       Next
    #Else  ' Windows
        ' --- CAMBIO PARA AUTOMATIZACIÓN ---
        FileList = Array(ThisWorkbook.Sheets(1).Range("C2").Value)
        Numfiles = 1
        
        ' También escríbelo en la hoja como si el usuario lo hubiese seleccionado
        MyCells.Offset(1, 0) = FileList(0)
        ' --- FIN DEL CAMBIO ---
    #End If
Else
...
#If Mac Then
    For I = 0 To Numfiles - 1  ' @RM Mac FileList variant is 0-based
        FileList(I) = MyCells(I + 2, 1).Value
        MyCells(I + 2, 1).Clear
    Next
#Else
    For I = 1 To Numfiles
...
Function SelectNMONFile() As String
  ...
  '  Dim mybook As Workbook
    ...
    ' allow selections of .csv or .nmon
    MyScript = _
    "set applescript's text item delimiters to "","" " & vbNewLine & _
               "set theFiles to (choose file of type " & _
             " {""public.comma-separated-values-text"", ""nmon""} " & _
               "with prompt ""Select nmon file(s) to be processed"" default location alias """ & _
               MyPath & """ multiple selections allowed true) as string" & vbNewLine & _
               "set applescript's text item delimiters to """" " & vbNewLine & _
               "return theFiles"

    SelectNMONFile = MacScript(MyScript)
End Function
...
#If Mac = False Then
    If Batch = 0 Then UserForm1.Show vbModeless  '@ RM 11/12/2015 hangs the script on MAC
#End If
    
If Merge <> "NO" Then Call MergeFiles(FileList, Numfiles)  ' @RM 11/12/2015 TODO MergeFiles won't work yet on MAC
...
' on Mac the FileList indexing is 0-based , on Windows it is 1-based
...
For I = startVal To endVal   ' @RM Mac FileList variant is 0-based
   'initialisation
   ErrMsg = ""
   FileName = FileList(I)
   Snapshots = 0
   ...
   Call SetStatus("Opening file " & Right(FileName, 40) & " (" & CStr(I) & " of " & CStr(Numfiles) & ")")
   
   'construct the output filename
   #If Mac Then
            ' @TW - 09/13/2017 - Fixing an issue with Excel 2016 & OSX 10.12.X, we only need the file name and not the whole path for singel file processing
            '    (otherwise it will create file like "/Users/username/Downloads/nmonfileXXXX.nmon.xlsx"
            ' @TW - 10/11/2017 - Fixing batch processing issue, where we need the full path again; Added .xlsx to the file name --> OSX doesn't need file extensions and by default Excel don't add them anymore
            '    This results in a situation, where Excel wants to overwrite the source file; Batch processing will write the target file in the same folder then the source file
            If CInt(Split(Application.Version, ".")(0)) >= 15 Then
                If Numfiles > 1 Or Batch = 1 Then
                    Output_Filename = FileName & ".xlsx"
                Else
                    Output_Filename = Right(FileName, Len(FileName) - InStrRev(FileName, "/", -1)) & ".xlsx" ' @TW - 09/13/2017 - Cut the file name for the dialog box
                End If
            Else
 ...
 ' @RM 7/13/17 - no longer include the output dir as the filename to avoid problems with Excel on Mac 2016
 '  If OutDir <> "" Then
 '     #If Mac Then
 ...
 '  End If
   ...
   ' End If
    ...
    Call SetStatus("Converting CSV format into columns on " + CStr(RawData.Sheets.Count) + " sheets")
    ...
    Call NMON(RawData, NmonFile)
         ...
         If NumCPUs > 0 Then
            ...
            Output_Filename = destino & fs.GetFileName(Output_Filename)
            ' === FIN BLOQUE DESTINO ===
...
EndLoop:
    ...
    If Merge = "YES" Then
        Call KillFileHelper(CStr(FileList(startVal)))   ' @RM MAC 0-based
    End If
...
Sub MergeFiles(FileList As Variant, Numfiles As Integer)
    ...
    Dim Tn As Long                         'current timestamp
    ...
    #If Mac Then
        ...
        startVal = 0
        endVal = Numfiles - 1
        ' @TW - 10/11/2017 - Change : to / for Excel 2016+ version to avoid grant access dialog for the users libary container path
        If CInt(Split(Application.Version, ".")(0)) >= 15 Then
        ...
        Call SetStatus(Right(FileList(startVal), 25))

        'Set fs = CreateObject("Scripting.FileSystemObject")
        'Set fnin = fs.OpenTextFile(FileList(startVal), 1, 0)
                                                  'extract only the sections we need
                                          
                                                                      'we have filelist - build a list of names
            Open FileList(startVal) For Input As #2
        ...
        For I = startVal + 1 To endVal  ' @RM Mac FileList variant is 0-based
   
           Call SetStatus(Right(FileList(I), 25))
   
           Open FileList(I) For Input As #2
...
finish:
        ...
        FileList(startVal) = aa0    ' @RM Mac FileList variant is 0-based
        Numfiles = 1
...
finish:
        ...
        d2 = Format(d2, "yymmdd")
        t2 = Format(t2, "hhmm")
        aa0 = hostname & "_" & d1 & "_" & t1 & "_to_" & d2 & "_" & t2

        aa0 = Replace(aa0, ":", "")  ' @RM remove any colons that may exist in the filename
        aa0 = Replace(aa0, ".", "")  ' @RM remove any dots that may exist in the filename
        aa0 = CurDir & "\" & aa0 & ".nmon"
...
Function KillFileHelper(Filestr As String)
...
Call SetStatus("Starting Analysis")
...
Do 'Until SectionName = ""
    ...
    If numrows > 1 And SectionName <> "" And SectionName <> "ERROR" Then
      Call SetStatus("Analysing: " & SectionName)
      ...
      On Local Error Resume Next ' @RM 2/17/2016 - added to help process huge data sets per user feedback
      ...
      ElseIf Left$(SectionName, 7) = "CPUUTIL" Then
         Call PP_CPUUTIL(numrows, Sheet1)
      ElseIf Left$(SectionName, 6) = "CPUMHZ" Then
         Call PP_CPUMHZ(numrows, Sheet1)
...
EndSect:
      ...
      If m_CurrentRow >= MaxRows And BigData Then
         m_StartRow = 1
         m_CurrentRow = 1
         On Error Resume Next ' @RM 2/17/2016 - added to help process huge data sets per user feedback
...
If OutputType <> "CHARTS" Then Call OutputPICS
...
If SheetExists("AAA") Then
    ...
    Section.Offset(1, 0) = "Analysis time"
    Section.Offset(1, 1) = Format(m_Elapsed, "#.00") & " seconds"
         
    Call SetStatus("Analysis Complete (" & m_Elapsed & " seconds)")
...
Call SetStatus("Converting charts to pictures")
...
ElseIf OutputType = "PRINT" Then
   ...
   If nc > 1 Then
      ...
      Call SetStatus("Adjusting horizontal page breaks")
...
ElseIf OutputType = "WEB" Then
   Call SetStatus("Generating HTML")
...
For I = numrows To 1 Step -1
    If Left(Labels.Cells(I, 1), 4) = "note" Then
        ...
        numrows = numrows - 1
    End If
    If Labels.Cells(I, 1) = "x86" Then
        Labels.Cells(I, 1) = "x86_" & I
    End If
Next I
...
' @RM 11/30/2020 - some Linux nmon files don't have progname field
On Error Resume Next
...
Sub PP_CPUUTIL(numrows As Long, Sheet1 As Worksheet)
...
'@RM - 2/22/2016 - check the flag to see if we should even bother
If (ShowLinuxCPUUtil = False And SectionName <> "CPUUTIL_ALL") Or _
 Application.WorksheetFunction.Max(Sheet1.Range("B2:IV" & CStr(numrows))) = 0 Then
   Application.DisplayAlerts = False
   Sheet1.Delete
   Application.DisplayAlerts = True
   Exit Sub
...
Set Graphdata = avgmax(numrows, Sheet1, SortDefault)
...
' @RM - 02/17/2016 - stacked bar portion (Y1) now only applies to Avg and WAvg (not the Max)
Set Graphdata = Sheet1.Range("A" & CStr(numrows + 2) & ":" & LastColumn & CStr(numrows + 3))


'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Error GoTo ForceTop1
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop1 + rH * numrows, cWidth, csHeight)
GoTo ResumeChart1

ForceTop1:
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop1, cWidth, csHeight)
ResumeChart1:
...
With Chart1.Chart                      'apply customisation
   ...
   Call MinMaxStyle(Chart1, Sheet1, numrows, SyncAxis)
End With
                                        'produce graph for all stats over time
Set Graphdata = Sheet1.Range("A1:" & LastColumn & CStr(numrows))

'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Error GoTo ForceTop2
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop2 + rH * numrows, cWidth, csHeight)
GoTo ResumeChart2

ForceTop2:
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop2, cWidth, csHeight)
ResumeChart2:

Chart1.Chart.ChartWizard Source:=Graphdata, Gallery:=xlColumnStacked, Format:=2, _
...
With Chart1.Chart
   ...
   .Axes(xlValue, xlPrimary).HasTitle = True
   .Axes(xlValue, xlPrimary).AxisTitle.Characters.Text = "Utilization (%)"
End With
...
Sub PP_CPUMHZ(numrows As Long, Sheet1 As Worksheet)
Dim Chart1 As ChartObject               'new chart object
Dim Graphdata As Range                  'range for charting
Dim SectionName As String
...
With Chart1.Chart
   ...
   .ChartGroups(1).GapWidth = 0
   .Axes(xlValue, xlPrimary).HasTitle = True
   .Axes(xlValue, xlPrimary).AxisTitle.Characters.Text = "CPU Frequency (MHz)"
End With
...
' Exclude CPUs from the graph (if it exists)
If Sheet1.Range("H1") = "CPUs" Then
    LastColumn = "G"
End If
If Sheet1.Range("G1") = "CPUs" Then
    LastColumn = "F"
End If
...
'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Error GoTo ForceTop1
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, cTop + rH * numrows, cWidth, csHeight)
GoTo ResumeChart1

ForceTop1:
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, cTop, cWidth, csHeight)
ResumeChart1:

Chart1.Chart.ChartWizard Source:=MyCells, Gallery:=xlColumnStacked, _
...
Call SetStatus("Creating: " & SectionName)
...
'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Error GoTo ForceTop1
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop1 + rH * NumCPUs, cWidth, csHeight)
GoTo ResumeChart1
...
If Not found Is Nothing Then
    ...
    Call SetStatus("Analysing: " & NewName)
...
If Len(SectionName) = 8 Then            'If this is the first DISKXFER Sheet
   ...
   Call SetStatus("Creating: " & "DISK_SUMM")
   
   ' added by Joe T. 02/26/2016
   On Local Error GoTo DS_Sheet_Modify
   ...
   If Graphs = "ALL" Or InStr(1, List, "DISK_SUMM") > 0 Then
    ...
    '@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
    On Local Error GoTo ForceTop1
    'Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop1 + rH * numrows, cWidth, csHeight)
    Set Chart1 = shDisk.ChartObjects.Add(cLeft, csTop1 + rH * numrows, cWidth, csHeight)
    GoTo ResumeChart1

ForceTop1:
    Set Chart1 = shDisk.ChartObjects.Add(cLeft, csTop1, cWidth, csHeight)
ResumeChart1:
    ...
    On Local Error GoTo 0
      ...
      End With
                                              'produce avg/max graph
      LastColumn = "D"
      Set MyCells = avgmax(numrows, shDisk, SortDefault)
      ' @RM - 02/17/2016 - stacked bar portion (Y1) now only applies to Avg and WAvg (not the Max)
      Set MyCells = shDisk.Range("A" & CStr(numrows + 2) & ":D" & CStr(numrows + 3))
    '
    ' @RM - 8/9/2019
    ' Sorting DISK_SUMM will cause problems with SYS_SUMM
    '  If SortDefault Then
    '   shDisk.Range("B1:" & LastColumn & CStr(numrows + 6)).Sort _
    '        Key1:=shDisk.Range("B" & CStr(numrows + 6)), Order1:=xlDescending, _
    '        Header:=xlYes, OrderCustom:=1, MatchCase:=False, Orientation:=xlLeftToRight
    '  End If
      
'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
    On Local Error GoTo ForceTop2
    Set Chart1 = shDisk.ChartObjects.Add(cLeft, csTop2 + rH * numrows, cWidth, csHeight)
    GoTo ResumeChart2

ForceTop2:
    Set Chart1 = shDisk.ChartObjects.Add(cLeft, csTop2, cWidth, csHeight)
ResumeChart2:
    ...
    On Local Error GoTo 0
      ...
      With Chart1.Chart                      'apply customisation
         ...
         Call ApplyStyle(Chart1, 0, 3)
         Call MinMaxStyle(Chart1, shDisk, numrows, SyncAxis)
      End With
   End If
...
If SectionName = "ESSXFER" And ESSanal Then
   Call SetStatus("Analysing ESS data")
...
Set MyCells = avgmax(numrows, Sheet1, False)
...
'@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Local Error GoTo ForceTop3
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop3 + rH * numrows, cWidth, csHeight)
GoTo ResumeChart3

ForceTop3:
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop3, cWidth, csHeight)
ResumeChart3:
On Local Error GoTo 0
...
If Graphs = "LIST" And Not Checklist(Sheet1.Name) Then Exit Sub
                                        'Put in the formulas for avg/max
Set Graphdata = avgmax(numrows, Sheet1, False)
NumAdapt = (ColNum - 1) / 5
                                        'produce avg/max graph
...
If Graphs = "LIST" And Not Checklist(Sheet1.Name) Then Exit Sub
                                        'produce avg/max graph
Set MyCells = avgmax(numrows, Sheet1, SortDefault)
' @RM - 02/17/2016 - stacked bar portion (Y1) now only applies to Avg and WAvg (not the Max)
Set MyCells = Sheet1.Range("A" & CStr(numrows + 2) & ":" & LastColumn & CStr(numrows + 3))
Column = LastColumn
...
With Chart1.Chart                      'apply customisation
    ...
    .Axes(xlValue).MaximumScale = 100
    Call ApplyStyle(Chart1, 0, 3)
    Call MinMaxStyle(Chart1, Sheet1, numrows, False)
End With
End Sub
                                       'v3.3.C
...
aa0 = ConvertRef(n)
Set Graphdata = Sheet1.Range(aa0 & CStr(numrows + 2) & ":" & LastColumn & CStr(numrows + 3))
...
If Graphs = "LIST" And Not Checklist(Sheet1.Name) Then Exit Sub
                                        'produce avg/max graph
Set MyCells = avgmax(numrows, Sheet1, False)
Set MyCells = Sheet1.Range("A" & CStr(numrows + 2) & ":C" & CStr(numrows + 3))
...
If numrows < 3 Then
   ...
   Exit Sub
End If
Call SetStatus("TOP: reorganizing data")
...
Call SetStatus("TOP: creating command table")

Application.Calculation = xlCalculationAutomatic
Application.Calculate

If numrows > 50000 Then
    Exit Sub
End If
...
If Scatter And Not topas And numrows <= 32000 Then
   ...
   '@RM - 2/11/2019 - Work around for overflow bug in MAC OS, this may cause graph to appear at top of sheet instead of bottom.
On Error GoTo ForceTop4
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop4 + rH * numrows, cWidth, csHeight)
GoTo ResumeChart4

ForceTop4:
Set Chart1 = Sheet1.ChartObjects.Add(cLeft, csTop4, cWidth, csHeight)
ResumeChart4:
...
For Each NextSheet In Worksheets
   ...
   Call SetStatus("Editing Time Values: " & MyName)
   ...
   Else
      If Left(NextSheet.Cells(2, 1).Value, 1) = "T" Then
         If CDbl(Mid(CStr(NextSheet.Cells(2, 1).Value), 2)) = CDbl(Mid(t1, 2)) Then
...
Else
   ...
   Sheet1.Range("L3").Value = "CPU%"
   Sheet1.Range("L4").Value = "=AVERAGE(CPU_ALL!J2:J" & aa0 & ")"
   Sheet1.Range("L5").Value = "=MAX(CPU_ALL!J2:J" & aa0 & ")"
End If
...
Sheet1.Range("B7").Value = "Total number of GBs read:"
'Sheet1.Range("E7").Value = "=SUM(DISK_SUMM!B2:B" & aa0 & ")*Interval/1000"
'@RM 11/4/2020 - the above Interval value doesn't take into account missed data so calculate # of seconds from the timestamps
'also dividing by 1048576 instead of 1000 to get GBs
Sheet1.Range("E7").Value = "=AVERAGE(DISK_SUMM!B2:B" & aa0 & ")*(DISK_SUMM!A" & aa0 & " - DISK_SUMM!A2) * 86400/1048576"
Sheet1.Range("E7").NumberFormat = "#,##0"

Sheet1.Range("B8").Value = "Total number of GBs written:"
'Sheet1.Range("E8").Value = "=SUM(DISK_SUMM!C2:C" & aa0 & ")*Interval/1000"
Sheet1.Range("E8").Value = "=AVERAGE(DISK_SUMM!C2:C" & aa0 & ")*(DISK_SUMM!A" & aa0 & " - DISK_SUMM!A2) * 86400/1048576"
...
Call SetStatus("Reordering sheets")
...
If Not m_GotESS Then
   ...
   For n = 1 To NumCPUs
      ...
      Call SetStatus("Reordering sheet: " & NextSheet.Name)
...
Else
   ...
   For Each NextSheet In Worksheets
      ...
      If Left$(MyName, 1) > "D" Or Left$(MyName, 2) = "DG" Then
         Call SetStatus("Reordering sheet: " & MyName)
      DoEvents
   ...
   Call SetStatus("Reordering DISK & Summary sheets")
...
Public Sub ProcessFile(RawData As Workbook)
    ...
    Else
        ...
        Call SetStatus("Reading " & FileName)
        ...
        Do Until blnEOF
            Call LoadFileBuffer(buffer, lngPos, outBuffer, blnEOF)
        Loop
        
        If buffer = "" Then
            Call SetStatus("No data detected when reading input file, aborting...")
            Exit Sub
        End If
        
        Call SetStatus("Dividing up data where vbLf occurs")
        ...
        Call SetStatus("Adding " & CStr(UBound(lines)) & " lines to sheet(s)")
        ...
        For X = 0 To UBound(lines)
            ...
            curLine = Trim(lines(X))
            
            ' add everything but blank lines and garbage lines (the extra split by empty string seems to take care of that check)
            If Len(Trim(curLine)) > 2 And Split(curLine, "")(0) <> "" Then
...
Sub SetStatus(Status As String)
    With Application
        ...
        .StatusBar = Status
        #If Mac Then
           If ShowMacProgress Then
' @TW - 2017/10/11 - Removed .ScreenUpdateing = True to increase the speed of the progress
'                .ScreenUpdating = True
                DoEvents
' @TW - 2017/10/11 - Removed .ScreenUpdateing = True to increase the speed of the progress
'                .ScreenUpdating = False
           End If
        #Else
            UserForm1.Label1.Caption = Status
            UserForm1.Repaint
        #End If
    End With
End Sub
...
Public Sub LoadFileBuffer(ByRef buffer As String, ByRef pos As Long, ByRef outBuffer, ByRef blnEOF As Boolean)
    On Error GoTo LastLine
    
    outBuffer = Mid$(buffer, pos, InStr(pos, buffer, vbNewLine) - pos)
    pos = InStr(pos, buffer, vbNewLine) + 2
